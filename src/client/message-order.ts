...
function turnTimestamp(turnId?: string): number {
  const match = String(turnId || '').match(/^voice-(\d+)-/)
  return match ? Number(match[1]) : Number.POSITIVE_INFINITY
}

export function insertByTurn(items: VoiceMessage[], message: VoiceMessage): VoiceMessage[] {
...
export function upsertUserTranscript(
  items: VoiceMessage[],
  input: Pick<VoiceMessage, 'id' | 'content' | 'turnId'> & { final?: boolean },
): VoiceMessage[] {
...
export function discardUserTranscript(items: VoiceMessage[], turnId?: string): VoiceMessage[] {
