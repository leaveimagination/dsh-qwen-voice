		...
		function QwenVoice(props) {
			...
			const updateTask = (0, react.useCallback)((event) => {
				...
				setTasks((current) => {
					...
					return sortTasks(current.findIndex((item) => item.id === next.id) < 0 ? [next, ...current] : current.map((item) => item.id === next.id ? {
						...item,
						...next
					} : item));
				});
			}, []);
			const refreshTasks = (0, react.useCallback)(async () => {
				try {
			...
			(0, react.useEffect)(() => {
				if (connected && open) refreshTasks();
			}, [
				connected,
				open,
				refreshTasks
			...
			(0, react.useEffect)(() => {
				...
				const connect = () => {
					...
					socket.onmessage = (message) => {
						...
						if (event.type === "transcript.discard" && event.role === "user") setMessages((items) => discardUserTranscript(items, event.turnId));
								...
								children: tasks.map((task) => (0, react_jsx_runtime.jsxs)("div", {
									...
									children: [
										...
										(0, react_jsx_runtime.jsxs)("span", { children: [
											...
											(0, react_jsx_runtime.jsxs)("small", { children: [
												(0, react_jsx_runtime.jsx)("em", { children: taskPhaseLabel(task.phase) }),
												task.createdAt ? (0, react_jsx_runtime.jsxs)("em", { children: [
													"· ",
													taskCreatedAtLabel(task.createdAt),
													" 创建"
												] }) : null,
												task.elapsedMs > 0 ? (0, react_jsx_runtime.jsxs)("em", { children: [
													"· ",
													Math.round(task.elapsedMs / 1e3),
													"s"
												] }) : null
											] })
										] }),
